...
// src/server.js
var {existsSync: existsSync4, readFileSync: readFileSync3, writeFileSync: writeFileSync3, mkdirSync: mkdirSync2, watch} = (() => ({}));
...
// src/lib/api.js
var {readFileSync: readFileSync2, writeFileSync: writeFileSync2, existsSync: existsSync3} = (() => ({}));

// src/lib/ontology.js
var {existsSync} = (() => ({}));

// src/lib/storage-format.js
var {readdirSync, readFileSync} = (() => ({}));
var ONTOLOGY_EXTENSIONS = new Set([".yml", ".yaml", ".md"]);
function isOntologyStorageFile(filePath) {
  return ONTOLOGY_EXTENSIONS.has(extname(filePath).toLowerCase());
}
function listOntologyFiles(storagePath) {
  const result = [];
  function walk(dir) {
...
function parseStorageFileContent(filePath, content) {
  ...
  if (extension === ".md") {
    ...
    const parsed = parse2(frontmatter);
    return {
      docs: parsed ? [parsed] : [],
      body
    };
  }
  const docs = content.split(/^---$/m).map((s) => s.trim()).filter(Boolean).map((s) => parse2(s)).filter(Boolean);
  return { docs, body: "" };
}
function parseStorageFile(filePath) {
  const content = readFileSync(filePath, "utf8");
  return parseStorageFileContent(filePath, content);
}
function serializeStorageFileContent(filePath, docs, body = "") {
  const extension = extname(filePath).toLowerCase();
  const nonNullDocs = (docs || []).filter(Boolean);
  if (extension === ".md") {
    const firstDoc = nonNullDocs[0] || {};
    const frontmatter = stringify3(firstDoc).trimEnd();
    return `---
${frontmatter}
---
${body || ""}`;
  }
  return nonNullDocs.map((d) => stringify3(d).trim()).join(`
---
`) + `
`;
}
function getRelativeStoragePath(storagePath, filePath) {
...
// src/lib/ontology.js
function loadOntology(storagePath) {
  if (!existsSync(storagePath)) {
    throw new Error(`Storage path not found: ${storagePath}`);
  }
  const files = listOntologyFiles(storagePath);
  const schema4 = { components: {}, classes: {}, relations: {} };
  const instances = [];
  for (const filePath of files) {
    const { docs, body } = parseStorageFile(filePath);
    const sourceFile = getRelativeStoragePath(storagePath, filePath);
    for (const doc of docs) {
      processDocument(doc, schema4, instances, sourceFile, body);
    }
  }
  return { schema: schema4, instances };
}
function processDocument(doc, schema4, instances, sourceFile, markdownBody = "") {
  if (!doc || doc.kind !== "Ontology")
    return;
  if (doc.schema) {
    mergeSchema(schema4, doc.schema);
  }
  if (doc.spec?.classes) {
...
function getClassColumns(schema4, className) {
  const classDef = schema4.classes[className];
  if (!classDef?.components)
    return [];
  const columns = [];
  for (const [localName, componentClass] of Object.entries(classDef.components)) {
    const component = schema4.components[componentClass];
...
function getClassRelations(schema4, className) {
  const relations = [];
  for (const [name, def] of Object.entries(schema4.relations || {})) {
...
function serializeInstance(instance, namespace) {
  const doc = {
    apiVersion: "agent/v1",
    kind: "Ontology",
    metadata: { namespace },
...
var {writeFileSync, unlinkSync, existsSync: existsSync2, mkdirSync} = (() => ({}));
function findInstanceFile(storagePath, instanceId, instanceClass) {
  ...
  for (const filePath of files) {
    ...
    for (const doc of docs) {
      if (!doc || doc.kind !== "Ontology")
        continue;
      const found = doc.spec?.classes?.some((c) => c._id === instanceId && c._class === instanceClass);
      if (found)
        return getRelativeStoragePath(storagePath, filePath);
    }
  }
  return null;
}
function saveInstance(storagePath, instance, namespace = "stormy") {
...
function updateInstanceInFile(filePath, instance, namespace) {
  ...
  for (const doc of docs) {
    if (!doc || doc.kind !== "Ontology")
      continue;
    if (doc.spec?.classes) {
      const idx = doc.spec.classes.findIndex((c) => c._id === instance._id && c._class === instance._class);
      if (idx !== -1) {
...
function deleteInstance(storagePath, instance) {
  const sourceFile = instance._source;
  if (sourceFile) {
    const sourceFilePath = join(storagePath, sourceFile);
    if (existsSync2(sourceFilePath)) {
...
function updateInstanceProperty(instance, columnId, value) {
...
function createNewInstance(className, id, columns) {
  const instance = {
    _class: className,
    _id: id,
    components: {},
...
// src/lib/api.js
function createAPI(storagePath) {
...
function readConfig(configPath) {
  ...
  const config = parse2(readFileSync2(configPath, "utf8")) || {};
  return normalizeConfig(config);
}
function writeConfig(configPath, config) {
  writeFileSync2(configPath, stringify3(config));
}
...
async function handleSaveViews(req, configPath) {
  try {
    ...
    let config = {};
    if (existsSync3(configPath)) {
      config = parse2(readFileSync2(configPath, "utf8")) || {};
    }
    config.views = views;
    writeFileSync2(configPath, stringify3(config));
...
var publicDir = resolve(projectRoot, "public");
var configPath = resolve(projectRoot, "config.yaml");
var config = loadServerConfig(configPath);
var storagePath = resolveStoragePath(config, projectRoot);
console.log("\uD83D\uDCC2 Storage path:", storagePath);
console.log("\uD83D\uDCC1 Public dir:", publicDir);
var api = createAPI(storagePath);
...
function loadServerConfig(path) {
  ...
  return parse2(readFileSync3(path, "utf8"));
}
function resolveStoragePath(cfg, root) {
  const storagePath2 = cfg.storage?.path || "../ontology/storage";
  return resolve(root, storagePath2);
}
...
`;
  ...
  writeFileSync3(bufferPath, bufferContent);
  const sandboxDir = resolve(projectRoot, "sandbox");
  if (!existsSync4(sandboxDir)) {
    mkdirSync2(sandboxDir, { recursive: true });
  }
  const finalCommand = command.replace(/\$BUFFER/g, bufferPath).replace(/\$AGENT/g, agentName);
